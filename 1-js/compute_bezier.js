var vxp = [],
	vyp = []; //lista punti curva in coord. floating point
var n = 4;
var m = 100; //per disegnare m+1

//algoritmo di de Casteljau per valutazione
//curva di Bézier di grado generico n in coordinate floating point
export function compute_bezier(fxt, fyt, sc, view, win) {
	// input
	...
	var h = 1 / m;
	var fxp = [],
		fyp = []; //copia dei cp per de Casteljau
	var t, d1, d2;
	...
	vxp[m] = fxt[n];
	vyp[m] = fyt[n];
	return win_view(vxp, vyp, sc.x, sc.y, view, win);
}
export function win_view(px, py, scx, scy, view, win) {
	const ixp = px.map((px) => Math.round(scx * (px - win.xmin) + view.xmin));
	const iyp = py.map((py) => Math.round(scy * (win.ymin - py) + view.ymax));
